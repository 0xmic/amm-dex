...
describe('AMM', () => {
  ...
  beforeEach(async () => {
    ...
    token1 = await Token.deploy('Crypto Token', 'CT', '1000000') // 1 Million Tokens
    token2 = await Token.deploy('USD Token', 'USD', '1000000') // 1 Million Tokens

    // Sent tokens to liquidity provider
  ...
  describe('Swapping Tokens', () => {
    ...
    it('facilitates swaps', async () => {
      ...
      expect(await amm.shares(deployer.address)).to.equal(tokens(100)) // user tokens helper to calculate shares
      ...
      console.log(`CT Price: ${(await amm.token2Balance() / await amm.token1Balance())}\n`)
      ...
      // Swap some more tokens to observe price change
      balance = await token2.balanceOf(investor1.address)
      console.log(`Investor1 Token2 balance before swap: ${ethers.utils.formatEther(balance)}`)
      ...
      balance = await token1.balanceOf(liquidityProvider.address)
      console.log(`LP Token1 balance before after liquidity: ${ethers.utils.formatEther(balance)}`)

      balance = await token2.balanceOf(liquidityProvider.address)
      console.log(`LP Token2 balance before after liquidity: ${ethers.utils.formatEther(balance)}\n`)
