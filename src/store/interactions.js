...
export const loadTokens = async (provider, chainId, dispatch) => {
  const ct = new ethers.Contract(config[chainId].ct.address, TOKEN_ABI, provider)
  const usd = new ethers.Contract(config[chainId].usd.address, TOKEN_ABI, provider)

  dispatch(setContracts([ct, usd]))
  dispatch(setSymbols([await ct.symbol(), await usd.symbol()]))
}
